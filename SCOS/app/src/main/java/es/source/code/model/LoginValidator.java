...
/**
 * Created by Liuhao32 on 2017/11/4.
 */
...
public class LoginValidator {
    ...
    public LoginValidator(String requestBody) {
        networkAsyncTask = new NetworkAsyncTask();
        networkAsyncTask.execute("NETWORK_POST_KEY_VALUE");
        try {
            this.requestBody = requestBody.getBytes("UTF-8");
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//            byte[] requestBody = null;//请求体
            ...
            try {
                if (NETWORK_GET.equals(action)) {
                    //发送GET请求
                    url = new URL("http://192.168.1.114:8080/SCOSServer/LoginValidator?name=liuhao&pwd=123");
                    ...
                    //用POST发送键值对数据
                    url = new URL("http://192.168.1.114:8080/SCOSServer/LoginValidator");
                    conn = (HttpURLConnection) url.openConnection();
...
//                    requestBody = new String("name=liuhao&pwd=123").getBytes("UTF-8");
                    //将请求体写入到conn的输出流中
...
//                    requestBody = getBytesFromAssets("person.json");
        ...
        protected void onPostExecute(Map<String, Object> result) {
            ...
            url = (String) result.get("url");//请求的URL地址
            ...
            String responseData = getStringByBytes(responseBody);
            ...
            try {
                ...
                msg = jsonObject.getString("RESULTCODE");
            ...
            if (requestHeader != null) {
                //  textView1.setText("请求头："+requestHeader);
            }
            ...
            if (NETWORK_GET.equals(action)) {
                ...
                //textView3.setText("返回："+response);
            } else if (NETWORK_POST_KEY_VALUE.equals(action)) {
                String response = getStringByBytes(responseBody);
                //textView3.setText("返回："+msg);
            } else if (NETWORK_POST_XML.equals(action)) {
                //将表示xml的字节数组进行解析
